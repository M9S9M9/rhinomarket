...
import { Search, Plus, Trash2, Shield, Key, Ban, CheckCircle, XCircle, FileText } from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const rows = filteredUsers.length > 0 ? filteredUsers.map((u) => (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name or email..." className="w-full rounded-lg border border-gray-300 pl-9 pr-3 py-2 text-sm focus:border-gray-500 focus:outline-none" />
      </div>
      ...
      <p className="text-sm text-gray-400 mb-3">{users.length} user{users.length !== 1 ? "s" : ""}</p>
