...
export default function DesignerDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (authStatus !== "authenticated" || !isDesigner) return;

    fetch("/api/dashboard/designer/stats")
      .then(r => r.json())
      .then(setStats)
      .catch(() => {})
      .finally(() => setStatsLoading(false));

    fetch(`/api/listings?designerId=${user.id}&limit=10`)
      .then(r => r.json())
  ...
  if (authStatus === "loading") return <div className="p-8 text-center text-gray-500">Loading...</div>;
  if (authStatus === "unauthenticated" || !session || !isDesigner) return null;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          ? Array.from({ length: 4 }).map((_, i) => (
              <Card key={i}><CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1"><Skeleton className="h-3 w-16 mb-1" /><Skeleton className="h-5 w-12" /></div>
                </div>
              </CardContent></Card>
            ))
          : (
      ...
      <Card className="mb-8">
        <CardContent className="p-6">
          ...
          {statsLoading ? (
            <Skeleton className="w-full h-2.5 rounded-full mb-4" />
          ) : (
        ...
        <CardContent className="p-6">
          ...
          {statsLoading ? (
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><Skeleton className="h-4 w-28 mb-2" /><Skeleton className="h-6 w-20" /></div>
              <div><Skeleton className="h-4 w-24 mb-2" /><Skeleton className="h-6 w-20" /></div>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-4">
